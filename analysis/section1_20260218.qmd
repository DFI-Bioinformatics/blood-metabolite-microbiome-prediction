...
(p1 / p2 / p3) + plot_layout(heights = c(9,3,3,2))
dev.off()
...
p_1d_2 = nmds_plot_df %>% 
  ...
  scale_shape_manual(values = cohort_shapes) +
  scale_fill_manual(values = cohort_colors) +
  labs(title = paste0("NMDS (Bray-Curtis) — stress=", round(nmds_stress, 2)), fill = NULL) +
  theme(panel.grid.minor = element_blank())
...
df_meta |> 
  distinct(study_id, age, sex, race, cci_score, ecog, induction_reg, mortality) |>
  ...
  tbl_summary(
    ...
    label = list(
      ...
      induction_reg ~ "Induction Regimen",
      mortality ~ "Mortality"
    )
  ) |> as_gt() |> gt::gtsave(here('output/tableS10_AML_summary.docx'))




df_meta_micu |> 
  distinct(sample_id, age, sex_cat, race, sofa_score, sepsis_cat, group) |>
  select(-sample_id) |>
  tbl_summary(
    statistic = list(
      age ~ "{median} ({p25}, {p75})",
    ...
    label = list(
      ...
      sepsis_cat ~ "Sepsis Type",
      group ~ "Mortality"
    )
  ) |> as_gt() |> gt::gtsave(here('output/tableS11_MICU_summary.docx'))
